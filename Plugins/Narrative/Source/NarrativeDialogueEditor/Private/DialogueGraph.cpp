	...
	if (UDialogueBlueprint* DialogueAsset = Cast<UDialogueBlueprint>(GetOuter()))
	{
		...
		if (UDialogueGraphNode_Root* RootGraphNode = Cast<UDialogueGraphNode_Root>(AddedNode))
		{
			if (!DialogueAsset->DialogueTemplate->RootDialogue)
			{
				...
				if (DialogueAsset->DialogueTemplate->Speakers.IsValidIndex(0))
				{
					RootNode->Text = FText::Format(LOCTEXT("DefaultRootNodeText", "Hi there, i'm {0}."), FText::FromString(DialogueAsset->DialogueTemplate->Speakers[0].SpeakerID.ToString()));
					RootNode->SpeakerID = DialogueAsset->DialogueTemplate->Speakers[0].SpeakerID;
				}
